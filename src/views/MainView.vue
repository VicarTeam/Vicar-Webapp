...
<!--      <Tab :value="1" :text="$t('main.tabs.vicarplay').toString()"/>-->
      ...
      <div style="position: absolute; right: 0.5rem; transform: translateY(-50%); top: 50%; z-index: 10" v-if="VicarPlayClient.isInSession()">
        <button class="btn btn-primary" style="max-width: 20rem; height: 4rem" @click="VicarPlayClient.isMenuOpen = !VicarPlayClient.isMenuOpen">
          {{$t('play.opensession')}}
        </button>
      </div>
    </Tabs>
