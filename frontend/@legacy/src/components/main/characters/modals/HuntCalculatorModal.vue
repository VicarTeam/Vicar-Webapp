...
export default class HuntCalculatorModal extends Vue {
  ...
  private get reduceToOptions(): IOption[] {
    ...
    for (let i = 0; i < this.character.hunger; i++) {
      options.push({value: i, name: this.$t('character.modal.hunt-calculator.reduce_to', {val: i}) as any});
    }
    return options;
  }
    ...
    return [
      {
        id: 0,
        max: 1,
        name: this.$t('character.modal.hunt-calculator.drink_opt.container'),
        time: () => this.$t('character.modal.hunt-calculator.drink_dur.round'),
      },
      {
        id: 1,
        max: 1,
        name: this.$t('character.modal.hunt-calculator.drink_opt.small_animal'),
        time: () => this.$t('character.modal.hunt-calculator.drink_dur.scene'),
      },
      {
        id: 2,
        max: 1,
        name: this.$t('character.modal.hunt-calculator.drink_opt.medium_animal'),
        time: () => this.$t('character.modal.hunt-calculator.drink_dur.round'),
      },
      {
        id: 3,
        max: 1,
        name: this.$t('character.modal.hunt-calculator.drink_opt.large_animal'),
        time: () => this.$t('character.modal.hunt-calculator.drink_dur.scene'),
      },
      {
        id: 4,
        max: 2,
        name: this.$t('character.modal.hunt-calculator.drink_opt.human'),
        name_hint: this.$t('character.modal.hunt-calculator.drink_opt.human.simple'),
        time: v => v === 1 ? this.$t('character.modal.hunt-calculator.drink_dur.rounds', {val: 3}) : this.$t('character.modal.hunt-calculator.drink_dur.scene'),
      },
      {
        id: 5,
        max: 4,
        name: this.$t('character.modal.hunt-calculator.drink_opt.human'),
        name_hint: this.$t('character.modal.hunt-calculator.drink_opt.human.danger'),
        time: v => this.$t('character.modal.hunt-calculator.drink_dur.rounds', {val: v}),
      },
      {
        id: 6,
        max: 5,
        name: this.$t('character.modal.hunt-calculator.drink_opt.human'),
        name_hint: this.$t('character.modal.hunt-calculator.drink_opt.human.fatal'),
        time: () => this.$t('character.modal.hunt-calculator.drink_dur.rounds', {val: 5}),
      }
    ];
  }
...
<template>
  <Modal :shown="show" @close="handleClose" v-if="character">
    <div class="w-600 d-flex justify-content-center align-items-center flex-column" style="gap: 0.5rem">
      <b>{{$t('character.modal.hunt-calculator', {name: character.name})}}:</b>

      <div style="display: flex; flex-direction: column; gap: 1rem; justify-content: flex-start; align-items: center">
        <span>{{$t('character.modal.hunt-calculator.current')}}: <b>{{character.hunger}}</b></span>
      ...
      <span>
        {{$t('character.modal.hunt-calculator.restriction')}}:
        <b>{{character.bloodPotency < 3 ? $t('character.modal.hunt-calculator.restriction.none') : $t('character.modal.hunt-calculator.restriction.living')}}</b>
        <TipButton v-if="character.bloodPotency >= 3" :content="$t('character.modal.hunt-calculator.restriction.hint')" style="margin-left: 0.5rem"/>
      </span>
      <div style="width: 100%; height: 1px; background-color: var(--primary-color); margin-top: 1rem"></div>
      ...
      <div v-if="character.hunger > 0" style="display: flex; flex-direction: column; gap: 1rem">
        <table class="table">
          <thead>
          <tr>
            <th>{{$t('character.modal.hunt-calculator.table.source')}}</th>
            <th>{{$t('character.modal.hunt-calculator.table.duration')}}</th>
            <th>{{$t('character.modal.hunt-calculator.table.amount')}}</th>
            <th>{{$t('character.modal.hunt-calculator.table.needed')}}</th>
          </tr>
          </thead>
          <tbody>
      ...
      <button class="btn" style="margin-top: 1rem" @click="showDetails = !showDetails">
        ...
        {{$t('character.modal.hunt-calculator.details')}}
      </button>

      <ul v-if="showDetails">
        <li v-for="detail in drinkDetails" :key="detail">
          {{$t('character.modal.hunt-calculator.details.' + detail)}}
        </li>
      </ul>
    </div>
  </Modal>
