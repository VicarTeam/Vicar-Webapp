...
export default class SearchHighlightModal extends Vue {
  ...
  private get options(): IOption[] {
    ...
    opts.push({
      name: this.$t('character.modals.search-highlight.stats').toString(),
      value: '',
      isCategory: true
    });
    opts.push({
...
<template>
  <Modal :shown="show" @close="show = false" v-if="character">
    <div class="w-400 d-flex justify-content-center align-items-center flex-column" style="gap: 0.5rem">
      <b>{{$t('character.modals.search-highlight')}}:</b>
      <Dropdown v-if="show" :options="options" v-model="highlight" :placeholder="$t('character.modals.search-highlight.search')" :autofocus="true"/>

      <div style="display: flex; flex-direction: row; gap: 1rem; justify-content: center; align-items: center; margin-top: 1.5rem">
        <button class="btn" @click="reset">
          {{$t('character.modals.search-highlight.reset')}}
        </button>
        <button class="btn btn-primary" @click="jump">
          {{$t('character.modals.search-highlight.jump')}}
        </button>
      </div>
    </div>
  </Modal>
