<template>
  <Modal :shown="show" @close="show = false">
    <div style="display: flex; flex-direction: column; gap: 1rem; width: 30rem">
      ...
      <div style="width: 100%; text-align: center" v-if="discipline"><span v-if="ability">{{ability.name}} <bullet/>{{' '}}</span>{{$t('viewer.modals.level.costs', {xp: neededExp})}}</div>
      <div style="width: 100%; display: flex; justify-content: center; align-items: center">
        <button class="btn btn-primary" :disabled="neededExp > editingCharacter.exp || !discipline || !ability" @click="level">{{$t('viewer.modals.level.btn')}}</button>
      </div>
    </div>
