  ...
  <Modal :shown="show" v-if="editingCharacter && data" @close="show = false">
    <div style="width: 60rem; display: flex; flex-direction: column; gap: 0.5rem">
      <div style="width: 100%; display: flex; gap: 1rem; align-items: center;">
        <b style="flex-grow: 1">{{$t('editor.traits.modals.title.' + (isFlaw ? 'flaw' : 'trait'))}}:</b>
        <select class="form-control categorized" v-model="selectedPack" @change="selectTrait(null)">
          ...
          <option style="font-style: italic; text-align: center" :value="_customPack">[GM] {{$t('editor.traits.modals.custom')}}</option>
        </select>
      </div>
        ...
        <div class="trait-pack-content">
          <div class="traits">
            <div class="trait" v-for="trait in traits" @click="selectTrait(trait)" :class="{'selected': trait === selectedTrait}">
              <b>{{trait.name}}</b> - <small><i><b>{{$t('editor.traits.modals.trait.level')}}</b>: {{trait.level}}</i></small>
            </div>
          </div>
          ...
          <div class="info" :class="{'not-selected': !selectedTrait}">
            <small v-if="!selectedTrait">{{$t('editor.traits.modals.info.notselected')}}</small>
      ...
      <div style="width: 100%; display: flex; gap: 2rem; flex-direction: column" v-else-if="selectedPack && selectedPack.id === _customPack.id">
        ...
        <div style="width: 100%; display: flex; flex-direction: column">
          <div class="form-group">
            <label>{{$t('editor.traits.modals.custom.type')}}:</label>
            <select class="form-control" v-model="customTraitType">
              <option value="merits">{{$t('editor.traits.modals.custom.type.merit')}}</option>
              <option value="backgrounds">{{$t('editor.traits.modals.custom.type.background')}}</option>
            </select>
          </div>
          <div class="form-group">
            <label>{{$t('editor.traits.modals.trait.level')}}:</label>
          ...
          <div class="form-group">
            <label>{{$t('editor.traits.modals.optionals.suffix')}}:</label>
      ...
      <div class="optional-trait-options" v-if="selectedTrait">
        <div class="form-group" style="width: 50%">
          <label>{{$t('editor.traits.modals.optionals.level')}}:</label>
      ...
      <div style="margin-top: 1rem; text-align: center; display: flex; flex-direction: column; justify-content: center; align-items: center">
        <span v-if="selectedTrait && calculateCosts" class="mb-10">{{$t('viewer.modals.level.costs', {xp: calculateCosts(this.selectedTrait, this.customLevel)})}}</span>
...
export default class ChooseTraitModal extends Vue {
  ...
  mounted() {
    ...
    this._customPack = {
      id: -42,
      type: "merits",
      name: this.$t("editor.traits.modals.custom").toString(),
      description: this.$t("editor.traits.modals.custom.desc").toString(),
