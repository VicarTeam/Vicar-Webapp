...
async function main() {
  ...
  const clanOptions = JSON.parse(gs).actions.filter((a: any) => a.agent.startsWith("option:clan:")).length;
  console.log(`[test] get_state -> ${clanOptions} clan options visible`);
  ...
  if (!folderOk || view !== "editor-clan" || clanOptions <= 0) {
    throw new Error("MCP smoke test assertions failed");
  }
  console.log("\n[test] SUCCESS - MCP client drove the real UI via the server (agent-token login)");
}
